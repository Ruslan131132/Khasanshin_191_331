...
protocol OAuthClient {
    ...
    func exchangeCodeForToken(code: String,
                              state: String,
                              completion: @escaping (Result<TokenBag, Error>) -> Void)//Метод извлечения парметров deeplink
}
...
class OAuthService {
    ...
    func exchangeCodeForToken(url: URL) {
        ...
        oauthClient.exchangeCodeForToken(code: code, state: state) { [weak self] result in //отсылаем обратно на Логин
            switch result {
