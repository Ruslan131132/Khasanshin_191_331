...
yandexdiskmodel::yandexdiskmodel(QObject *parent) : QAbstractListModel(parent)
{

}

void yandexdiskmodel::addItem(const FileObject &newItem)
{
    ...
    beginInsertRows(QModelIndex(), rowCount(), rowCount());  //начальный номер строки вставки и конечный номер строки вставки
    objectlist << newItem;  //вставка нового элемента
    endInsertRows();  //сообщает ListView о том, что изменение модели закончено
}

int yandexdiskmodel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return objectlist.count();
}

QVariant yandexdiskmodel::data(const QModelIndex &index, int role) const
{
...
QVariantMap yandexdiskmodel::get(int idx) const
{
...
QHash<int, QByteArray> yandexdiskmodel::roleNames() const
{
